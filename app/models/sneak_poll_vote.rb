class SneakPollVote < ActiveRecord::Base
  ...
  named_scope :blank,         :conditions => ["#{quoted_table_name}.timeliness IS NULL AND #{quoted_table_name}.quality IS NULL AND #{quoted_table_name}.commitment IS NULL AND #{quoted_table_name}.office_procedures IS NULL AND (#{quoted_table_name}.notes IS NULL OR #{quoted_table_name}.notes = '')"]
  named_scope :unblank,       :conditions => ["#{quoted_table_name}.timeliness IS NOT NULL OR #{quoted_table_name}.quality IS NOT NULL OR #{quoted_table_name}.commitment IS NOT NULL OR #{quoted_table_name}.office_procedures IS NOT NULL OR (#{quoted_table_name}.notes IS NOT NULL AND #{quoted_table_name}.notes <> '')"]
  ...
  def css_classes
    ...
    classes << (notes.blank? ? 'notes-blank' : 'notes-voted')
